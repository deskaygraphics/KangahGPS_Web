...
with tab_about:
    ...
    st.markdown(
        """
        **KangahGPS** is a coordinate conversion tool built for Ghana, with support
        for both **Molodensky (3-parameter)** and **Bursa-Wolf (7-parameter)** datum
        transformation methods between the War Office ellipsoid and WGS 84.

        #### Bursa-Wolf 3D Coordinate Transformation
        The Bursa-Wolf (Helmert) 7-parameter transformation is a rigorous 3D similarity
        transformation that accounts for **three translations** (dX, dY, dZ), **three
        rotations** (rX, rY, rZ), and a **scale factor** (dS). This method provides
        higher accuracy (~1 m) compared to the simpler 3-parameter Molodensky approach (~6 m).

        The parameters used in KangahGPS for the War Office → WGS 84 transformation are:
        - **Molodensky (3-param):** dX = −170, dY = 33, dZ = 326
        - **Bursa-Wolf (7-param):** dX = −171.16, dY = 17.29, dZ = 325.21, rX = 0, rY = 0, rZ = 0.814, dS = −0.38

        Users can select the transformation method in the **Settings** sidebar.
